...
### v1.75
- **Every finding now reaches the Burp Dashboard** — findings with no underlying HTTP exchange (out-of-band **TLS Analyzer** checks, and any async/Collaborator finding reported without its probe request) were being silently dropped because the reporter required a request/response to anchor the issue. They now get a minimal synthetic request built from the finding's URL, so passive **and** active findings consistently appear in Dashboard → Issue Activity and the Site Map.
